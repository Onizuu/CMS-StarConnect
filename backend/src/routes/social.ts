...
router.post('/connect/twitter', authenticate, async (req: AuthRequest, res) => {
    try {
...
// Disconnect account
router.delete('/account/:id', authenticate, async (req: AuthRequest, res) => {
    try {
...
router.patch('/account/:id/toggle', authenticate, async (req: AuthRequest, res) => {
    try {
        const account = await prisma.socialAccount.findUnique({
            where: { id: req.params.id },
        });
