...
router.post('/now', authenticate, async (req: AuthRequest, res) => {
    try {
        ...
        const result = await SocialPublishService.publishToSocial(
            req.user!.userId,
            contentId,
            platforms
        );
...
router.delete('/:id', authenticate, async (req: AuthRequest, res) => {
    try {
        await SocialPublishService.cancelQueueItem(
            req.params.id,
            req.user!.userId
        );
